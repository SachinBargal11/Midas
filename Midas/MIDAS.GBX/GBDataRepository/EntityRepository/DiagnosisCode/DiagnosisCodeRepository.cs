...
namespace MIDAS.GBX.DataRepository.EntityRepository
{
    internal class DiagnosisCodeRepository : BaseEntityRepo, IDisposable
    {
        ...
        public override T Convert<T, U>(U entity)
        {
            ...
            diagnosisCodeBO.ICDTypeCodeID = diagnosisCode.ICDTypeCodeID;
        ...
        {
            var acc_ = _context.DiagnosisCodes.Include("ICDTypeCode").Where(p => p.IsDeleted == false || p.IsDeleted == null).ToList<DiagnosisCode>();
            ...
            List<BO.DiagnosisCode> lstDiagnosisCode = new List<BO.DiagnosisCode>();

            foreach (DiagnosisCode item in acc_)
            {
                lstDiagnosisCode.Add(Convert<BO.DiagnosisCode, DiagnosisCode>(item));
            }
        ...
        {                                                         
            var boDiagnosisCodeDB = from dc in _context.DiagnosisCodes
                                    join dcc in _context.DiagnosisCodeCompanies on dc.Id equals dcc.DiagnosisCodeID
                                    where
                                    dcc.DiagnosisTypeCompnayID == DiagnosisTypeId
