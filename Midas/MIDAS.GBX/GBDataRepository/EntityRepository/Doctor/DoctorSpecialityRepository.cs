...
namespace MIDAS.GBX.DataRepository.EntityRepository
{
    internal class DoctorSpecialityRepository : BaseEntityRepo, IDisposable
    {
        ...
        public override T ObjectConvert<T, U>(U entity)
        {
            ...
            if (doctorspecility.Specialty.IsDeleted.HasValue == false || (doctorspecility.Specialty.IsDeleted.HasValue == true && doctorspecility.Specialty.IsDeleted.Value == false))
            {
                BO.Specialty boSpecliality = new BO.Specialty();
                using (SpecialityRepository sr = new SpecialityRepository(_context))
                {
