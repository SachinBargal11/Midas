    ...
    internal class InvitationRepository : BaseEntityRepo
    {
        ...
        public override T Convert<T, U>(U entity)
        {
            ...
            if (invitation.User.UserCompanies != null && invitation.User.UserCompanies.Count > 0)
            {
                ...
                invitation.User.UserCompanies.Where(p => p.IsAccepted == true && (p.IsDeleted.HasValue == false || p.IsDeleted == null ))
                                  .ToList().
                                  ForEach(x => boUserCompany.Add(
                                      new BO.UserCompany()
                                      { CompanyId = x.CompanyID,
                                          UserId = x.UserID,
                                          UserStatusID = (BO.GBEnums.UserStatu)x.UserStatusID,
                                          CreateByUserID = x.CreateByUserID,
                                          ID = x.id,
                                          IsDeleted = x.IsDeleted,
                                          UpdateByUserID = x.UpdateByUserID
                                      }));
