            ...
            using (MessagingEntities db = new MessagingEntities())
            {
                ...
                if (emailConfiguration != null)
                {
                    var messagingemailConfiguration = new Entities.EMailConfiguration
                    {
                        ...
                        FromEmailID = emailConfiguration.FromEmailID
                    };
            ...
            using (MessagingEntities db = new MessagingEntities())
            {
                ...
                if (emailqueue != null)
                {
                    var messagingemailqueue = new Entities.EMailQueueItem
                    {
                        ...
                        ApplicationName = emailqueue.ApplicationName,
                        FromEmail = emailqueue.FromEmail,
                        ToEmail = emailqueue.ToEmail,
                        CcEmail = emailqueue.CcEmail,
                        BccEmail = emailqueue.BccEmail,
            ...
            using (MessagingEntities db = new MessagingEntities())
            {
                ...
                if (smsqueue != null)
                {
                    var messagingsmslqueue = new Entities.SMSQueueItem
                    {
                        ...
                        ApplicationName = smsqueue.ApplicationName,
                        ToNumber = smsqueue.ToNumber,
                        FromNumber = smsqueue.FromNumber,
                        Message = smsqueue.Message
                    };
        ...
        public void AddEmailToQueue(Entities.EMailQueueItem queue)
        {
            using (MessagingEntities db = new MessagingEntities())
            {
                EMailQueue mailqueue = new EMailQueue
                {
                    ...
                    ToEmail = queue.ToEmail,
                    CcEmail = queue.CcEmail,
                    BccEmail = queue.BccEmail,
                    EMailSubject = queue.EMailSubject,
                    EMailBody = queue.EMailBody
                };
        ...
        public DataOperationStatus UpdateEmailToQueue(Entities.EMailQueueItem queue)
        {
            using (MessagingEntities db = new MessagingEntities())
            {
                ...
                if(mailqueue!= null)
                {
                    ...
                    mailqueue.ApplicationName = queue.ApplicationName;
                    mailqueue.FromEmail = queue.FromEmail;
                    mailqueue.ToEmail = queue.ToEmail;
                    mailqueue.CcEmail = queue.CcEmail;
                    mailqueue.BccEmail = queue.BccEmail;
                    mailqueue.EMailSubject = queue.EMailSubject;
                    mailqueue.EMailBody = queue.EMailBody;
        ...
        public DataOperationStatus UpdateSMSToQueue(Entities.SMSQueueItem queue)
        {
            using (MessagingEntities db = new MessagingEntities())
            {
                ...
                if (smsqueue != null)
                {
                    ...
                    smsqueue.ApplicationName = queue.ApplicationName;
                    smsqueue.ToNumber = queue.ToNumber;
                    smsqueue.FromNumber = queue.FromNumber;
                    smsqueue.Message = queue.Message;
