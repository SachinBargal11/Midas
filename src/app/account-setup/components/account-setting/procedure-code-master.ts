...
export class ProcedureCodeComponent implements OnInit {
    ...
    loadAllSpecialitiesAndTests() {
        ...
        let fetchAllSpecialities = this._specialityStore.getSpecialitiesBycompnayId(this._sessionStore.session.currentCompany.id);
        let fetchAllTestFacilties = this._roomsStore.getTestsByCompanyId(this._sessionStore.session.currentCompany.id);
    ...
    setPreffferedProcedureMappings(currentprocedure:Procedure, updatestatus:string) {
        if (currentprocedure.id > 0) {
          if(updatestatus =='set')
          {
          ...
          else if(updatestatus =='reset')
          {
            ...
            this.confirmationService.confirm({
                ...
                accept: () => {
                        this.isDeleteProgress = true;
                        this._progressBarService.show();
                        this._procedureCodeMasterStore.UpdatePreffredProcedureMapping(currentprocedure)
                            .subscribe(
                            (response) => {
                                let notification = new Notification({
