...
export class DiagnosisService {
    ...
    getDiagnosisTypeByCompanyId(): Observable<DiagnosisType[]> {
        ...
        let promise: Promise<DiagnosisType[]> = new Promise((resolve, reject) => {
            return this._http.get(environment.SERVICE_BASE_URL + '/DiagnosisTypeCompany/getByCompanyId/' + companyId, {
                headers: this._headers
            })
                .map(res => res.json())
    ...
    getDiagnosisCode(): Observable<DiagnosisCode[]> {
        let promise: Promise<DiagnosisCode[]> = new Promise((resolve, reject) => {
        return this._http.get(environment.SERVICE_BASE_URL + '/DiagnosisCode/getAll', {
                headers: this._headers
            }).map(res => res.json())
                .subscribe((data: any) => {
                    resolve(data);
