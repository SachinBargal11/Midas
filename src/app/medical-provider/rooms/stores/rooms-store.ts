...
export class RoomsStore {
    ...
    getTestsByCompanyId(companyId: number): Observable<Tests[]> {
        let promise = new Promise((resolve, reject) => {
            this._roomsService.getTestsByCompnayId(companyId).subscribe((tests: Tests[]) => {
                this._tests.next(List(tests));
                resolve(tests);
            }, error => {
                reject(error);
    ...
    findTestSpecialityById(id: number): Tests {
        let testSpecialities = this._tests.getValue();
        let index = testSpecialities.findIndex((currentTestSpeciality: Tests) => currentTestSpeciality.id === id);
        return testSpecialities.get(index);
    }

    fetchTestSpecialityById(id: number): Observable<Tests> {
        let promise = new Promise((resolve, reject) => {
            let matchedTestSpeciality: Tests = this.findTestSpecialityById(id);
            if (matchedTestSpeciality) {
                resolve(matchedTestSpeciality);
            } else {
                this._roomsService.getTestsByID(id)
                    .subscribe((testSpeciality: Tests) => {
