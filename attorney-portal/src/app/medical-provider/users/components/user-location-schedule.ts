...
@Component({
    selector: 'user-location-schedule',
    templateUrl: './user-location-schedule.html'
})

export class UserLocationScheduleComponent implements OnInit {
    schedules: Schedule[];
    userId: number;
    ...
    scheduleJS: any;
    userLocationScheduleDetail: UserLocationSchedule;
    ...
    constructor(
        ...
        this._route.parent.params.subscribe((params: any) => {
            ...
            let fetchUserLocationSchedule = this._userLocationScheduleStore.getUserLocationSchedule(scheduleId);

            Observable.forkJoin([fetchSchedules, fetchUserLocationSchedule])
                .subscribe((results) => {
                    this.userLocationScheduleDetail = results[1];
                    this._fetchScheduleWithDetails(this.userLocationScheduleDetail.schedule.id);
                },
                (error) => {
    ...
    updateSchedule() {
        ...
        result = this._scheduleStore.updateSchedule(schedule).flatMap((schedule: Schedule) => {
            return this._userLocationScheduleStore.updateUserScheduleForLocation(this.userLocationScheduleDetail, schedule);
        });
        result.subscribe(
            (response) => {
    ...
    enableAssign() {
        ...
        result = this._userLocationScheduleStore.updateUserScheduleForLocation(this.userLocationScheduleDetail, this.currentSchedule);
        result.subscribe(
            (schedule) => {
                let notification = new Notification({
                    'title': 'Schedule updated successfully!',
