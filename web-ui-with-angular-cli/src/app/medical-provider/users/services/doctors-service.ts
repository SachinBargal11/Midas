...
export class DoctorsService {
    ...
    getDoctorByIdandCompnayID(doctorId: Number,companyId: number): Observable<Doctor> {
        let requestData = {
            companyId: companyId,
            CurrentUserId: doctorId
        }
        let promise: Promise<Doctor> = new Promise((resolve, reject) => {
            return this._http.post(environment.SERVICE_BASE_URL + '/Doctor/getDoctorByIDAndCompnayID/' , JSON.stringify(requestData), {
                headers: this._headers
            }).map(res => res.json())
                .subscribe((data: any) => {
                    let parsedDoctor: Doctor = null;
    ...
    getReadingDoctorsByCompanyId(companyId: number): Observable<Doctor[]> {
        let promise: Promise<Doctor[]> = new Promise((resolve, reject) => {
            return this._http.get(environment.SERVICE_BASE_URL + '/Doctor/getReadingDoctors/' + companyId, {
                headers: this._headers
            }).map(res => res.json())
                .subscribe((data: Array<Object>) => {
                    let doctors = (<Object[]>data).map((doctorData: any) => {
    ...
    getReadingDoctorsByCompanyLocationRoomTestId(companyId: number, locationId: number, testId: number): Observable<Doctor[]> {
        let promise: Promise<Doctor[]> = new Promise((resolve, reject) => {
            return this._http.get(environment.SERVICE_BASE_URL + '/Doctor/getReadingDoctors/' + companyId +'/'+ locationId +'/'+ testId, {
                headers: this._headers
            }).map(res => res.json())
                .subscribe((data: Array<Object>) => {
                    let doctors = (<Object[]>data).map((doctorData: any) => {
    ...
    getDoctorsTaxType(): Observable<any[]> {
        let promise: Promise<any[]> = new Promise((resolve, reject) => {
            return this._http.get(environment.SERVICE_BASE_URL + '/Doctor/getDoctorTaxTypes', {
                headers: this._headers
            }).map(res => res.json())
                .subscribe((data: any[]) => {
                    resolve(data);
