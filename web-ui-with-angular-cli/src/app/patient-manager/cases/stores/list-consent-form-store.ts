...
export class ListConsentStore {
    ...
    getConsetForm(CaseId: number,companyId: number): Observable<ListConsent[]> {

        let promise = new Promise((resolve, reject) => {
            this._listconsentFormService.getConsetForm(CaseId,companyId).subscribe((consent: ListConsent[]) => {
    ...
    deleteUploadConsetForm(caseDetail: ListConsent): Observable<ListConsent> {
        let cases = this._ListConsent.getValue();
        let index = cases.findIndex((currentCase: ListConsent) => currentCase.id === caseDetail.id);
        let promise = new Promise((resolve, reject) => {
            this._listconsentFormService.deleteUploadConsentform(caseDetail).subscribe((caseDetail: ListConsent) => {
                this._ListConsent.next(cases.delete(index));
                resolve(caseDetail);
            }, error => {
                reject(error);
